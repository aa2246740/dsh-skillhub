...
export function apply(ctx: Context, config: Config) {
  ...
  const handler = handleSkillHubHttp(hub, invalidate)
  ...
  ctx.effect(() => {
    ...
    return onCreated('agent/created', payload => { attachAgentProvider(hub, payload) })
  }, 'dsh-skillhub agent provider')
  console.log('[my-plugins/dsh-skillhub] http /skillhub')
}

function attachAgentProvider(hub: SkillHub, payload: unknown): void {
  if (typeof payload !== 'object' || payload === null) return
  const agent = (payload as { agent?: { ctx?: Context } }).agent
